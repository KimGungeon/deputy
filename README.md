...
**조율자 한 명이 기본으로 들어간다.** 담당 디렉터리 없이 전체를 보는 역할이다.
담당자는 자기 영역만 보므로, 두 제안이 같은 파일을 건드리는지·순서가 뒤집혔는지·
어떤 작업이 멈춰 있는지는 아무도 못 본다. 조율자가 그걸 본다.

| 역할 | 담당 디렉터리 | 하는 일 |
|---|---|---|
| 담당자 `builder` | 있음 | 제안 → 합의 → 구현 → 완료 |
| 조율자 `coordinator` | **없음** | 리뷰·중재·백로그 관리. 구현하지 않음 |

조율자는 담당이 없으므로 **파일 충돌을 일으킬 수 없고**, 담당자가 둘뿐일 때
실질적인 정족수를 만들어 준다 (리뷰 1건 → 2건). `deputy claim` 은 거부된다.
빼려면 `--no-coordinator`.
...
```bash
deputy 7          # #7 에 대해 지금 할 일을 판단하고 실행
deputy 7 --dry    # 실행하지 않고 무엇을 할지만 본다
```

상태를 읽어 다음 행동을 정한다. **인자가 필요 없는 행동은 바로 실행**하고
(착수, 합의된 분해안 적용), 판단이 필요한 것은 붙여넣을 명령을 출력한다
(제안, 리뷰, 완료, 파생).

| 이슈 상태 | `deputy <번호>` 가 하는 일 |
|---|---|
| 제안 없음 | 크기를 판단해 제안 또는 분해하라고 안내 |
| 남의 제안이 검토 대기 | 리뷰 명령 출력 |
| 내 제안이 합의됨 | **착수 자동 실행** |
| 내 제안에 이견 | 반대 사유를 보여주고 수정 명령 출력 |
| 분해안이 합의됨 | **자식 이슈 생성 자동 실행** |
| 계획으로 내게 배정됨 | **착수 자동 실행** (재합의 불필요) |
| 내가 착수해 진행 중 | 완료·파생 명령 출력 |
| 남이 착수함 | 건드리지 말라고 안내 |
| 에픽 | 자식 진행률, 전부 닫혔으면 닫기 안내 |
...
deputy 는 **분해 도구가 아니라 처리량 도구**다. 잘 잘린 이슈를 주면 갈아내지만
"음성 대화 기능 넣어줘" 같은 요청은 받지 못한다. 그 간극을 메우는 것이 `deputy plan` 이다.

```bash
deputy plan --template            # 형식과 규칙
deputy plan 7 --file plan.json    # 분해안 제출 (검증 후 합의 대기)
deputy plan 7 --apply             # 합의되면 실제 이슈로 생성
```

분해는 합의를 거친다. 파생 이슈는 이미 합의된 완료 기준이 강제하는 사실이라
합의를 건너뛰지만, 분해는 여러 갈래 중 하나를 고르는 선택이고 틀리면 8시간이 날아간다.

제출 시 자동 검증되며 **건너뛸 수 없다**: 담당자가 실재하는가(조율자 불가),
담당이 한 명에게 몰리지 않는가, 완료 기준이 명령과 기대 결과로 쓰였는가,
선행 관계에 순환이 없는가, 자식이 12개를 넘지 않는가.

적용되면 자식 이슈는 재합의 없이 바로 착수 가능하고, 선행 작업이 안 끝난 것은
`deputy next` 가 걸러준다. 에픽은 자식이 모두 닫혀야 닫힌다.
...
| 명령 | 하는 일 |
|---|---|
| `deputy plan <n>` | 큰 이슈를 실행 가능한 작은 이슈로 분해 |
| `deputy scan` | 저장소 구조를 훑어 멤버 구성 제안 (변경 없음) |
| `deputy <번호>` | 그 이슈에 대해 지금 할 일을 판단하고 자명한 것은 실행 |
